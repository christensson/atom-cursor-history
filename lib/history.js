...
module.exports = class History {
  ...
  showInEditor(editor) {
    ...
    entries.forEach((entry, index) => {
      this.highlightEntry(editor, entry, index)
    })
  }
  ...
  highlightEntry(editor, entry, index) {
    console.log(`Highlight ${index}: ${entry.inspect()}`)
    let marker = editor.markBufferPosition(entry.point)

    let element = document.createElement("span")
    element.className = "badge badge-small cursor-history-overlay"
    element.textContent = `${index}`

    const lineClass = entry.isEdited() ? "cursor-history-highlight-line-edited" : "cursor-history-highlight-line"
    editor.decorateMarker(marker, {type: "line", class: lineClass})
    editor.decorateMarker(marker, {type: "overlay", position: "head", item: element})
