...
module.exports = class History {
  ...
  init() {
    this.index = 0
    this.entries = []
    this.lineHighlightDisposables = []
  }

  clear() {
    this.entries.forEach(destroyEntry)
    this.lineHighlightDisposables.forEach((item) => item.dispose())
    this.init()
  }
  ...
  destroy() {
    ...
    this.lineHighlightDisposables = null
  }
  ...
  findAllInEditor(editor) {
    return this.entries.slice().filter((entry) => {
      if (!entry.isValid()) return false
  ...
  add(location, {setIndexToHead = true, message} = {}) {
    ...
    if (atom.config.get("cursor-history.keepSingleEntryPerBuffer")) {
      ...
      let atSameRow = this.entries.filter(isAtSameRow)
      if (!newEntry.isEdited() && atSameRow.some((entry) => entry.isEdited())) {
        newEntry.setEdited()
      }
      atSameRow.forEach(destroyEntry)
    }
  ...
  showInEditor(editor, minimap) {
    const uri = editor.getPath()
    console.log(`Show entries in ${uri}`)
    ...
    const entries = this.findAllInEditor(editor)

    if (entries.size == 0) {
      this.log("No history entries in current editor")
      return
    }
    ...
    console.log(`Entries in editor ${uri}`)
    this.lineHighlightDisposables.forEach((item) => item.dispose())
    this.lineHighlightDisposables = []
    entries.forEach((entry, index) => {
      this.highlightEntry(editor, minimap, entry, index)
    })
  }
  ...
  highlightEntry(editor, minimap, entry, index) {
    let marker = editor.markBufferPosition(entry.point)

    let lineClass = "cursor-history-highlight-line"
    if (entry.isEdited()) {
        lineClass += " edited"
    }
    editor.decorateMarker(marker, {type: "line", class: lineClass})
    let minimapDecoration
    if (minimap) {
        let minimapLineClass = "cursor-history-highlight-line"
        if (entry.isEdited()) {
            minimapLineClass += " edited"
        }
        minimapDecoration = minimap.decorateMarker(marker, {
            type: "line", class: minimapLineClass
        })
    }
    ...
    const destroyMarker = () => {
      ...
      if (minimapDecoration) {
          minimapDecoration.destroy()
          minimapDecoration = null
      }
    }

    disposable = editor.onDidChangeCursorPosition(destroyMarker)
    // [NOTE] animation-duration has to be shorter than this value
    setTimeout(destroyMarker, 4000)
    this.lineHighlightDisposables.push({
        dispose: destroyMarker
    })
  }

  markEdit(location) {
    const editEntry = new Entry(location)
    const editedEntry = this.entries.find(editEntry.isAtSameRow.bind(editEntry))
    if (editedEntry) {
      console.log(`Edit of history entry: ${editedEntry.inspect()}`)
      editedEntry.setEdited()
    } else {
      console.log(`Edit location has no history: ${editEntry.inspect()}`)
      // this.add(location, {message: "Edit at cursor"})
    }
  }
}
